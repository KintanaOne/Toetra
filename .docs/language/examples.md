...
Compare two related points and ensure that the score does not decrease.
...
The language supports explicit attributes, but the current comparison representation should be reviewed if right-hand side attribute comparisons are required. The AST currently represents comparison right operands primarily as constants. Supporting attribute-vs-attribute comparisons may require extending the AST node model.
