...
from tests.support.e2e_point_binding import build_linear_artifacts, source


def test_two_point_universal_property_proves_positive_slope(tmp_path) -> None:
...
def test_alternating_quantifiers_are_rejected_before_translation(
    tmp_path,
) -> None:
    artifacts = build_linear_artifacts(
        tmp_path,
    ...
    task = run_ir2_with_model_schema(
        source("alternating_quantifiers"),
        schema=schema,
    )[0]
