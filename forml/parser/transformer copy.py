...
class FORMLTransformer(Transformer):
    ...
    def property(self, items):
        ...
        check_expr = next((i for i in items if isinstance(i, CheckExpr)), None)
        pairwise_expr = next((i for i in items if isinstance(i, PairwiseExpr)), None)
    ...
    def problem_expr(self, items):
        ...
        func_tree = items[1]
        if isinstance(func_tree, Tree) and func_tree.data == "function_expr":
            function = self.function_expr(func_tree.children)
        elif isinstance(func_tree, FunctionExpr):
            function = func_tree
        else:
